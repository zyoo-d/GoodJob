														...
														<form>
															<div class="input-group">
																<a href="#" class="btn btn-icon icon-left btn-primary"
																	...
																	onclick="SelectReview()"><i class="far fa-edit"></i>
																	일괄 승인</a>
															</div>
														</form>
													</div>
															...
															<c:forEach items="${list}" var="dto">
																<c:if test="${dto.cp_rv_confirm == 0}">
																	<tr>
																		...
																		<td class="p-0 text-center">${dto.cp_rv_seq}
																		</td>
																		<td class="p-0 text-center">${dto.cp_name}</td>
																		<td class="p-0 text-center"><a
																				href="/good/user/company/review/editreview.do?cp_rv_seq=${dto.cp_rv_seq}">${dto.id}</a>
																		</td>
																		<td class="p-0 text-center">${dto.cp_rv_regdate}
																		</td>
																		<td class="p-0 text-center status"
																			data-toggle="modal"
																			data-target="#reviewModal"
																			data-cp-rv-seq="${dto.cp_rv_seq}"
																			style="cursor: pointer;">
																			<c:if test="${dto.cp_rv_confirm == 0}">대기
																			</c:if>
																			<c:if test="${dto.cp_rv_confirm == 1}">승인
																			</c:if>
																			<c:if test="${dto.cp_rv_confirm == 2}">거절
																			</c:if>
																		</td>
																	</tr>
																</c:if>
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<form id="reviewStatusForm" method="post" action="/good/admin/review/review.do">
								<input type="hidden" name="cp_rv_seq" id="cp_rv_seq">
								<div class="form-group">
									<label for="reviewStatus">상태:</label> <select class="form-control" id="reviewStatus"
								...
								<div class="form-group" id="rejectReasonGroup" style="display: none;">
									...
									<textarea class="form-control" id="rejectReason" name="rj_reason" rows="3"
										required></textarea>
								</div>
							</form>
						</div>
