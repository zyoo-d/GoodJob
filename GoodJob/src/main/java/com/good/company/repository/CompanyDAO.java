...
/**
 * CompanyDAO 클래스는 기업 관련 데이터베이스 작업을 수행합니다.
 */
public class CompanyDAO {
	...
	/**
	 * 메인 페이지에 표시할 기업 목록을 조회합니다.
	 * 
	 * @param map 검색 조건
	 * @return 평점 3.5점 이상의 기업 정보 목록
	 */
	...
	public ArrayList<CompanyDTO> comListInfo(HashMap<String, String> map) {
		...
		try {
			...
			sql = "SELECT * FROM (SELECT a.*, ROWNUM AS rnum FROM (SELECT * FROM vwListCompany "
					+ (where.isEmpty() ? "" : "WHERE " + where) + " ORDER BY " + orderBy + ") a) WHERE rnum BETWEEN "
					+ map.get("begin") + " AND " + map.get("end");
	...
	public int getCompanyCount(HashMap<String, String> map) {
		try {
			...
			sql = "SELECT COUNT(*) AS cnt FROM vwListCompany " + (where.isEmpty() ? "" : "WHERE " + where);
	 ...
	 * 실시간 댓글을 추가하는 메서드
	 * 
	 * @param cmdto 댓글 정보를 담은 CommentDTO 객체
	 * @return 추가된 행의 수
	 */
	 ...
	 * 기업 매칭 정보를 조회하는 메서드
	 * 
	 * @param mdto 매칭 정보를 담은 MatchingDTO 객체
	 * @return 기업 정보를 담은 CompanyDTO 객체
	 */
	...
	/**
	 * 데이터 업데이트 날짜를 조회하는 메서드
	 * 
	 * @return 데이터 업데이트 날짜 정보를 담은 HashMap
	 */
	...
	/**
	 * 기업 시퀀스로 등록 개수가 상위인 태그를 조회하는 메서드
	 * 
	 * @param cp_seq 기업 시퀀스
	 * @return 상위 태그 목록
	 */
	...
	/**
	 * 리뷰 수 기준 상위 5개 기업을 조회하는 메서드
	 * 
	 * @return 리뷰 수 기준 상위 5개 기업 정보를 담은 LinkedHashMap
	 */
	public LinkedHashMap<String, Integer> getTop5CompaniesByReviewCount() {
		...
		try {
			...
			String sql = "SELECT cp_name, cnt " + "FROM (" + "    SELECT cp_name, cnt, ROWNUM AS rnum " + "    FROM ("
					+ "        SELECT c.cp_name, COUNT(*) AS cnt " + "        FROM tblCompanyReview cr "
					+ "        INNER JOIN tblCompany c ON cr.cp_seq = c.cp_seq "
					+ "        GROUP BY c.cp_seq, c.cp_name " + "        ORDER BY cnt DESC " + "    )" + ")"
					+ " WHERE rnum <= 5";
	...
	/**
	 * 리뷰 점수 기준 상위 5개 기업을 조회하는 메서드
	 * 
	 * @return 리뷰 점수 기준 상위 5개 기업 정보를 담은 LinkedHashMap
	 */
	public LinkedHashMap<String, Double> getTop5CompaniesByReviewScore() {
		...
		try {
			...
			String sql = "SELECT rnum, cp_name, avg " + "FROM (" + "    SELECT ROWNUM AS rnum, cp_name, avg "
					+ "    FROM (" + "        SELECT cp_name, avg " + "        FROM vwCompanyReviewAvg "
					+ "        ORDER BY avg DESC " + "    )" + ")" + "WHERE rnum <= 5";
	 ...
	 * 연도별 리뷰 통계를 조회하는 메서드
	 * 
	 * @return 연도별 리뷰 통계 정보를 담은 LinkedHashMap
	 */
