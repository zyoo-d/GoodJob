...
public class CompanyModal extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {		
		...
		// cp 배열을 request 객체에 저장하여 JSP에서 사용할 수 있도록 함
		req.setAttribute("selectedCp", cp);
		...
		// cp 배열을 쿼리스트링으로 변환
		String cpQueryString = "";
		if (cp != null) {
		    for (int i = 0; i < cp.length; i++) {
		        cpQueryString += "&compareCp=" + cp[i];
		    }
		}
		...
		if (n == 1) {
		    ...
		    sb.append(String.format(
		            "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'><span class='material-symbols-outlined paging-icon z-custom'>navigate_before</span></a></li>",
		            n - 1, hiring, word, cpQueryString));
		} else if (n > 5) {
		    sb.append(String.format(
		            "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'><span class='material-symbols-outlined paging-icon z-custom'>keyboard_double_arrow_left</span></a></li>",
		            n - 5, hiring, word, cpQueryString));
		    sb.append(String.format(
		            "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'><span class='material-symbols-outlined paging-icon z-custom'>navigate_before</span></a></li>",
		            n - 1, hiring, word, cpQueryString));
		}
		...
		while (!(loop > blockSize || n > totalPage)) {
		    if (n == nowPage) {
		        ...
		        sb.append(String.format(
		                "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'>%d</a></li>",
		                n, hiring, word, cpQueryString, n));
		    }
		    loop++;
		    n++;
		}
		...
		if (n >= totalPage) {
		    ...
		    sb.append(String.format(
		            "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'><span class='material-symbols-outlined paging-icon z-custom'>navigate_next</span></a></li>",
		            n, hiring, word, cpQueryString));
		    sb.append("<li class='page-item z-custom'><a class='page-link' href='#!'><span class='material-symbols-outlined paging-icon z-custom'>keyboard_double_arrow_right</span></a></li>");
		} else {
		    sb.append(String.format(
		            "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'><span class='material-symbols-outlined paging-icon z-custom'>navigate_next</span></a></li>",
		            n, hiring, word, cpQueryString));
		    sb.append(String.format(
		            "<li class='page-item z-custom'><a class='page-link' href='/good/user/company/cp_selectModal.do?page=%d&hiring=%s&word=%s%s'><span class='material-symbols-outlined paging-icon z-custom'>keyboard_double_arrow_right</span></a></li>",
		            n + 5, hiring, word, cpQueryString));
		}
