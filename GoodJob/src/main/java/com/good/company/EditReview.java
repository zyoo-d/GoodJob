...
public class EditReview extends HttpServlet {
	 ...
	 * GET 요청 처리 메소드
	 * 
	 * 리뷰 수정 페이지를 로드합니다. 세션에서 사용자 ID를 가져오고, 요청 파라미터에서 리뷰 시퀀스를 가져옵니다. 리뷰 정보와 기업 정보를
	 * 조회하여 JSP 페이지로 전달합니다. 관리자 여부와 작성자 여부를 확인하여 JSP 페이지에 전달합니다.
	 * 
	 ...
	 * POST 요청 처리 메소드
	 * 
	 * 리뷰 수정 요청을 처리합니다. 세션에서 사용자 ID를 가져오고, 요청 파라미터에서 액션, 리뷰 시퀀스, 태그 키워드, 삭제할 태그 등을
	 * 가져옵니다. 리뷰 정보와 태그 정보를 조회하고, 해시태그 수정 및 삭제 작업을 수행합니다. 리뷰 수정 로직을 처리하고, 수정된 정보를
	 * 데이터베이스에 업데이트합니다.
	 * 
	 * @param req  HttpServletRequest 객체
	 * @param resp HttpServletResponse 객체
